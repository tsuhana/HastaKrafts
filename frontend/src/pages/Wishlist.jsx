...
const Wishlist = () => {
  ...
  // Pagination
  const totalPages    = Math.ceil(wishlist.length / WISHLIST_PER_PAGE);
  const pagedWishlist = useMemo(
    () => wishlist.slice((page - 1) * WISHLIST_PER_PAGE, page * WISHLIST_PER_PAGE),
    [wishlist, page]
  );
  ...
  const handleAddToCart = async (item) => {
    ...
    setAddingToCart(prev => ({ ...prev, [item.product_id]: true }));
    try {
      await cartAPI.addToCart({ product_id: item.product_id, quantity: 1 });
      ...
      setAddingToCart(prev => ({ ...prev, [item.product_id]: false }));
    }
  };
  ...
  const confirmConfig = {
    ...
    addAll: {
      title: `Add ${confirmModal.itemCount} items to cart?`,
      message: 'All available items will be added to your cart.',
      confirmText: t('wishlist.add_all'),
      confirmVariant: 'warning',
    },
    clear: {
      title: 'Clear entire wishlist?',
      message: 'All items will be removed. This cannot be undone.',
      confirmText: t('wishlist.clear'),
      confirmVariant: 'danger',
    },
  };
  ...
  return (
    <div className="wishlist-page">
      ...
      <div className="wishlist-container">
        ...
        {/* Grid — only show current page items */}
        ...
        {totalPages > 1 && (
          <div style={{ marginTop: '2rem' }}>
            ...
            <p style={{ textAlign: 'center', fontSize: '0.8rem', color: '#9a8268', marginTop: '0.5rem' }}>
              Showing {(page - 1) * WISHLIST_PER_PAGE + 1}–{Math.min(page * WISHLIST_PER_PAGE, wishlist.length)} of {wishlist.length} items
            </p>
          </div>
        )}
      </div>
